...
def add_parser(subparsers) -> argparse.ArgumentParser:
    parser = subparsers.add_parser(
        NAME,
        help="Query standard UDS identity DIDs (part no., serial, VIN, ...) from --tx ECU",
        description="Query standard UDS identity DIDs (F100, F18x, F190, F19x) and decode them.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="examples:\n  canair identity --tx 770 --session --wake\n",
    )
    parser.add_argument("--tx", metavar="ID", required=True, help="ECU TX ID (hex, e.g. 770)")
