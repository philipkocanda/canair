...
| VCU       | 0x7E2  | 🔶 Partial   | 2101 working (gear, vehicle state, speed). Speed is in MPH — raw field is MPH×100 big-endian at `[B20:B21]`, expression converts to km/h; confirmed vs dashboard at 3 points (+5.1% speedo over-read), GPS/multi-point verification still pending. PIDs 2102 captured but not decoded (motor heat sink temp, inverter voltage, MCU temp, motor RPM). Regen mode and ECO/Sport/Normal TODO. |
...
- [ ] **VCU speed** — unit resolved as MPH (converted to km/h in expression); still verify with GPS across full range + confirm `[B20:B21]` low byte, and validate ESC `22C101` `REAL_SPEED_KMH` (B12) as the true dashboard speed source (only captured at standstill so far)
