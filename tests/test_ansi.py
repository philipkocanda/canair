...
zero escapes) is a separate integration test, added in commit 4.
"""
...
class TestPalette:
    def test_palette_matches_the_codes_the_tree_actually_used(self) -> None:
        ...
        assert ansi.RED == "\033[91m"
        assert ansi.GREEN == "\033[92m"
        assert ansi.YELLOW == "\033[93m"
        assert ansi.CYAN == "\033[96m"
...
class TestC:
    def test_wraps_when_colour_on(self) -> None:
        out = ansi.c("hi", ansi.YELLOW, stream=_FakeStream(isatty=True))
        assert out == f"{ansi.YELLOW}hi{ansi.RESET}"

    def test_returns_plain_when_colour_off(self) -> None:
        assert ansi.c("hi", ansi.YELLOW, stream=_FakeStream(isatty=False)) == "hi"

    def test_no_color_disables_wrapping(self, monkeypatch) -> None:
        monkeypatch.setenv("NO_COLOR", "1")
        assert ansi.c("hi", ansi.YELLOW, stream=_FakeStream(isatty=True)) == "hi"
...
class TestCerr:
    def test_gated_on_stderr_not_stdout(self, monkeypatch) -> None:
        ...
        assert ansi.cerr("warn", ansi.RED) == "warn"
    ...
    def test_colours_when_stderr_is_a_tty(self, monkeypatch) -> None:
        ...
        assert ansi.cerr("warn", ansi.RED) == f"{ansi.RED}warn{ansi.RESET}"
