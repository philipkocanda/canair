...
| Parameters | **354** decoded (**227 verified**, 127 unverified/candidate) |
| IOControl DIDs | **107** actuators (UDS `0x2F`) |
| Research backlog | **58** open reverse-engineering leads |
<!-- END generated-stats -->

> These numbers are generated by `scripts/gen_profiles_index.py` (CI checks they
> are current), derived from the same data as
> `canair --profile ioniq-2017 validate pids --stats` and `canair research`.
