...
```
usage: canair identity [-h] [--session] [--wake] [--protocol {auto,uds,kwp}] [--wican WICAN] [--transport {slcan-tcp,wican-ws}] [--elm-timeout MS] [--timeout SECONDS] [--json] [--verbose]
...
options:
  ...
  --protocol {auto,uds,kwp}
                        Identity protocol: uds (22 F1xx), kwp (1A 8x/9x KWP2000), or auto (registry hint, else on-device probe). Default: auto
