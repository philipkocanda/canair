...
- **`wican.py`** — WiCAN device management CLI. Subcommands: `config` (view/save device config), `sleep` (toggle sleep mode, set voltage/timing), `reboot`. Uses `--wican home|vpn|<url>` for address selection. Dependencies: `requests`.
