#!/usr/bin/env python3
"""Safely add/update pids/ parameters and research entries from the CLI.
...
engineering workflow. Every edit is:
  ...
  2. schema-checked with validate-pids.py — and reverted if validation fails.

This makes it safe for an agent to record a decoded parameter or track research
progress without hand-editing the rich per-ECU YAML by hand.
...
Examples:
  ...
  python3 pids-edit.py upsert-param MCU 2102 MCU_MOTOR_TORQUE "[S12:S13]/100" \\
  ...
  python3 pids-edit.py add-research MCU --type decode --target 2103 \\
      --status captured --priority P2 --prereq charging --notes "27 bytes, zeros"
  python3 pids-edit.py set-status MCU 2103 done --type decode
"""
...
VALIDATE_SCRIPT = ROOT / "validate-pids.py"
...
def _schema_validate(fpath: Path) -> tuple[bool, str]:
    """Run validate-pids.py on a single file. Returns (ok, output)."""
    proc = subprocess.run(
        [sys.executable, str(VALIDATE_SCRIPT), str(fpath)],
...
def _guarded(ecu: str, pids_dir: Path, do_edit, *, validate: bool):
    ...
    The library layer already reverts on YAML corruption; this adds the
    schema gate (unknown fields, bad expressions, PCI-byte spans, etc.).
    """
...
def _add_common(sp: argparse.ArgumentParser) -> None:
    sp.add_argument("--dir", type=Path, default=PIDS_DIR,
                    help=f"pids/ directory (default: {PIDS_DIR})")
    sp.add_argument("--no-validate", action="store_true",
                    help="Skip the post-edit validate-pids.py schema gate")


def cmd_upsert_param(args: argparse.Namespace) -> None:
    def do():
        upsert_parameter(
    ...
    print(f"{_GREEN}  ✓ {args.ecu} {args.pid} {args.name}{_RESET}  "
          f"{_DIM}({fpath.name}){_RESET}")


def cmd_add_research(args: argparse.Namespace) -> None:
    def do():
        add_research_entry(
    ...
    fpath = _guarded(args.ecu, args.dir, do, validate=not args.no_validate)
    print(f"{_GREEN}  ✓ research {args.ecu} {args.type} {args.target} "
          f"[{args.status}]{_RESET}  {_DIM}({fpath.name}){_RESET}")


def cmd_set_status(args: argparse.Namespace) -> None:
    def do():
        set_research_status(args.ecu, args.target, args.status,
                            type=args.type, pids_dir=args.dir)
    fpath = _guarded(args.ecu, args.dir, do, validate=not args.no_validate)
    print(f"{_GREEN}  ✓ {args.ecu} research {args.target} -> {args.status}{_RESET}  "
          f"{_DIM}({fpath.name}){_RESET}")
...
def main() -> None:
    ...
    _add_common(up)
    up.set_defaults(func=cmd_upsert_param)
    ...
    ar.add_argument("--status", required=True,
                    choices=["pending", "captured", "nrc", "done"])
    ar.add_argument("--priority", choices=["P1", "P2", "P3"])
    ar.add_argument("--prereq", action="append",
    ...
    _add_common(ar)
    ar.set_defaults(func=cmd_add_research)
    ...
    ss.add_argument("--type", choices=["scan", "decode", "verify", "iocontrol_scan"],
