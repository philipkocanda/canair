...
- **`canair validate`** — Schema validation with a positional target: `canair validate pids|captures|ecus|all` (default `all`). Add `--stats` for a count summary (ECUs/PIDs/params/verified/research items). Schemas are tool-owned in `canlib/schema/` (`pids_schema.yaml`, `captures_schema.json`, `ecus_schema.yaml`); `pids/_meta.yaml` stays in the profile
