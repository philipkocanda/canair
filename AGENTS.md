...
- **`canair dtc`** — Read stored Diagnostic Trouble Codes (DTCs) for one ECU or `--all` (UDS `0x19` / KWP2000 `0x18`, auto-selected by `id_protocol`). Logs each scan to the profile's `dtc_log.yaml` and reports what **changed** since the last scan; clear fault memory with `--clear` (`0x14`). Read-only unless `--clear` is passed.
...
- **`canair ecu`** — Inspect the profile's ECU registry (`ecus.yaml`) and per-ECU stats (PID/param/verified counts, protocol, addresses). Companion to `canair validate ecus`.

### Profiles

Vehicle data lives in a **profile** bundle — a directory with `pids/`, `ecus.yaml`, `captures/`, `states.yaml`, and generated `out/`. The repo ships `profiles/ioniq-2017/` as the default/example. Selection precedence: `--profile NAME|PATH` (global flag, before the subcommand) > `CANAIR_PROFILE` env var > `default_profile` in config > single discovered profile (auto). Profiles are discovered from `--profiles-dir`, `$CANAIR_PROFILES_DIR`, `profiles_dir` in config, `~/.config/canair/profiles/` (user, uncommitted), and the repo's bundled `profiles/`; user profiles shadow bundled ones by name. Inspect with `canair profile list` / `show [NAME]` / `path [NAME]`. Scaffold a new vehicle with `canair profile create <name> --car-model "…"` (creates `pids/_meta.yaml`, empty `ecus.yaml`, `captures/`, `out/` under `~/.config/canair/profiles/<name>` or `--path DIR`; `--set-default` to adopt it).

## Key Files

- **`profiles/ioniq-2017/pids/`** — SOURCE OF TRUTH for all PID definitions, split by ECU (one file per ECU). Validate with `canair validate pids`; run `canair validate pids --stats` for current ECU/PID/parameter/verified counts
- **`canair validate`** — Schema validation with a positional target: `canair validate pids|captures|ecus|states|all` (default `all`). Add `--stats` for a count summary (ECUs/PIDs/params/verified/research items). Schemas are tool-owned in `canlib/schema/` (`pids_schema.yaml`, `captures_schema.json`, `ecus_schema.yaml`, `states_schema.yaml`); `pids/_meta.yaml` stays in the profile. `validate captures` also emits **soft warnings** for session `state`s outside the profile's `states.yaml` vocabulary (free text still accepted).
...
- **`profiles/ioniq-2017/research/`** — External reference material (other-vehicle logs, spreadsheets, SavvyCAN exports, charge-curve data) used to cross-reference RE work. Distinct from the per-ECU `research:` backlog sections inside `pids/` (surfaced by `canair research`).
