...
| Subcommand | Purpose |
|--------|---------|
| `canair captures` | Search/diff saved diagnostic captures, or `--step` through them (several PIDs stacked in one time-joined frame) (`captures uds`); infer & back-fill missing session states (`--backfill-states`); list raw-CAN frame logs (`captures can`) (alias: `cap`). |
